...
def from_ref(arg_list):
    ...
    prefix = string.upper(arg_list[0])

    if len(prefix) != 2:
	print "Grid reference %s does not make sense"%string.join(arg_list)
...
def main():
    ...
    if arg_len == 2:
	try:
	    east  = string.atoi(arg_list[0],10)
	    north = string.atoi(arg_list[1],10)
	except:
	    print __doc__
	    return
	to_ref(east,north)
